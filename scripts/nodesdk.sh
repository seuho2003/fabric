...
NUM_FILES_CHANGED=$(git diff --name-only $GIT_COMMIT origin/$GERRIT_BRANCH | egrep 'sdk/node' | wc -l)
NUM_PROTO_FILES_CHANGED=$(git diff --name-only $GIT_COMMIT origin/$GERRIT_BRANCH | egrep 'proto$' | wc -l)
git diff --name-only $GIT_COMMIT origin/$GERRIT_BRANCH | egrep 'sdk/node'
git diff --name-only $GIT_COMMIT origin/$GERRIT_BRANCH | egrep 'proto$'
echo "Git Commit" $GIT_COMMIT
echo "Target Branch" $GERRIT_BRANCH
