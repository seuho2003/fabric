...
git log --name-only $TRAVIS_COMMIT_RANGE | grep "sdk/node"
...
NUM_FILES_CHANGED=$(git log --name-only $TRAVIS_COMMIT_RANGE | grep "sdk/node" | wc -l)
echo "Number of files changed in sdk/node directory:" $NUM_FILES_CHANGED
NUM_PROTO_FILES_CHANGED=$(git log --name-only $TRAVIS_COMMIT_RANGE | grep "protos" | wc -l)
